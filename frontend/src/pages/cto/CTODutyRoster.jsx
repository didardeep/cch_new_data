        ...
        <div className="section-card-body" style={{ display: 'grid', gridTemplateColumns: '3fr 2fr', gap: 18 }}>
          <div>
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 12 }}>
              ...
              <div style={{ fontSize: 12, color: '#0f172a', fontWeight: 700, background: '#e2e8f0', padding: '4px 10px', borderRadius: 999 }}>
                24x7 Coverage
              </div>
            </div>
            ...
            {!rosterLoading && !rosterError && rosterData?.meta && (
              <div style={{ display: 'flex', gap: 12, flexWrap: 'wrap', marginBottom: 12 }}>
                <div style={{ background: '#f1f5f9', padding: '6px 10px', borderRadius: 8, fontSize: 12, fontWeight: 700 }}>
                  ...
                  <table style={{ width: '100%', borderCollapse: 'collapse', tableLayout: 'fixed' }}>
                    <thead>
                      <tr style={{ background: 'linear-gradient(90deg, #f8fafc 0%, #eef2ff 100%)' }}>
                        <th style={{ width: '25%', textAlign: 'left', padding: '14px 16px', fontSize: 12, fontWeight: 800, color: '#0f172a', borderRight: '1px solid #d7dee8', borderBottom: '1px solid #d7dee8' }}>
                          Off Today
                        </th>
                        {[0, 1, 2].map(i => (
                          <th key={i} style={{ width: '25%', textAlign: 'left', padding: '12px 16px', borderRight: i < 2 ? '1px solid #d7dee8' : 'none', borderBottom: '1px solid #d7dee8' }}>
                    ...
                    <tbody>
                      {Array.from({ length: maxRows }).map((_, rowIdx) => (
                        <tr key={rowIdx}>
                          <td style={{ width: '25%', verticalAlign: 'top', padding: 8, borderRight: '1px solid #d7dee8', borderBottom: rowIdx < maxRows - 1 ? '1px solid #eef2f7' : 'none', background: '#fff7ed' }}>
                            {offToday[rowIdx] ? (
                              <div style={{ display: 'grid', gridTemplateColumns: '28px 1fr', gridTemplateRows: 'auto auto', alignItems: 'center', columnGap: 8, rowGap: 2, background: '#fff', padding: '8px 10px', borderRadius: 12, border: '1px solid #fed7aa', minHeight: 80 }}>
                          ...
                          {shiftLists.map((list, idx) => (
                            <td key={idx} style={{ width: '25%', verticalAlign: 'top', padding: 8, borderRight: idx < 2 ? '1px solid #d7dee8' : 'none', borderBottom: rowIdx < maxRows - 1 ? '1px solid #eef2f7' : 'none', background: '#ffffff' }}>
                              {list[rowIdx] ? (
                                <div style={{ display: 'grid', gridTemplateColumns: '28px 1fr', gridTemplateRows: 'auto auto', alignItems: 'center', columnGap: 8, rowGap: 2, background: '#f8fafc', padding: '8px 10px', borderRadius: 12, border: '1px solid #e2e8f0', minHeight: 80 }}>
                                  <div style={{ gridRow: '1 / span 2', width: 28, height: 28, borderRadius: '50%', background: '#e2e8f0', color: '#0f172a', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 10, fontWeight: 800 }}>
                                    {initials(list[rowIdx].name)}
                                  </div>
                                  <div style={{ minWidth: 0 }}>
                                    <div style={{ fontSize: 12, fontWeight: 700, color: '#0f172a', lineHeight: '16px', wordBreak: 'break-word' }}>
                                      {list[rowIdx].name}
                                    </div>
                                    <div style={{ fontSize: 10, color: '#64748b', lineHeight: '14px' }}>
          ...
          <div style={{ border: '1px solid #d7dee8', borderRadius: 16, padding: 18, background: '#ffffff', boxShadow: '0 10px 24px rgba(15, 23, 42, 0.08)' }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(7, 1fr)', gap: 10, marginBottom: 10 }}>
              {['Sun', 'Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat'].map(d => (
                <div key={d} style={{ textAlign: 'center', fontSize: 12, fontWeight: 700, color: '#64748b' }}>{d}</div>
              ))}
            </div>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(7, 1fr)', gap: 10 }}>
              {weeks.map((week, wi) => (
                week.map((cell, ci) => {
                  ...
                  const baseColor = cell.inMonth ? '#ffffff' : '#e2e8f0';
                  return (
                    <button
                      key={`${wi}-${ci}`}
                      ...
                      style={{
                        padding: '12px 0',
                        borderRadius: 10,
                        border: isSelected ? '2px solid #00338D' : '1px solid #d7dee8',
                        background: isSelected ? '#e0e7ff' : baseColor,
                        color: cell.inMonth ? '#0f172a' : '#94a3b8',
