...
# ─────────────────────────────────────────────────────────────────────────────
# GET /api/network/ai-query  (unchanged core logic — extended with new KPI cols)
# ─────────────────────────────────────────────────────────────────────────────
@network_bp.route("/api/network/ai-query", methods=["POST"])
@jwt_required()
def ai_query():
    uid = get_jwt_identity()
    user = db.session.get(User, int(uid))
    ...
    time_filter = _dynamic_time_filter(time_range)
    ...
    # Maps natural-language terms (EN/HI) to exact kpi_name in database
    SCHEMA_HINT = """
Tables:
...
4. SQL PERFORMANCE — MANDATORY:
   ...
   - ALWAYS: AND k.kpi_name IN ('exact_name_1', 'exact_name_2')
   - For single KPI + single site trend: use WHERE k.kpi_name = 'X' directly (no CASE WHEN needed)
   - For multiple KPIs: use CASE WHEN pivot pattern
   - LIMIT max 100 rows
...
- "composed" → Two different metrics on same time axis (e.g. PRB + throughput over time)
                USE THIS for: "show X and Y together over time"
...
{{
  ...
  "filter_update": {{}}
        ...
        if "```" in raw:
            ...
            for part in parts[1:]:
                ...
                if part.startswith("{"):
                    raw = part
                    break
        # If still has leading text before JSON, find the first {
        idx = raw.find("{")
        if idx > 0:
            raw = raw[idx:]
    ...
    def _parse_ai_result(raw_text):
        ...
        # Normalise chart_type / query_type (LLM may use either key)
    ...
    # Reads keys at request time so the same code works on any system:
    #   - System with AZURE_OPENAI_API_KEY + AZURE_OPENAI_ENDPOINT  → Azure OpenAI
    #   - System with GEMINI_API_KEY                                → Google Gemini
    #   - System with OPENAI_API_KEY                                → OpenAI direct
    #   - No keys at all                                            → rule-based fallback
    # If multiple are configured, tries each in order until one succeeds.
    ...
    # Build ordered provider list: try whichever keys are present
    _providers = []
    ...
    for _prov in _providers:
        ...
        try:
            ...
            continue  # skip retry, go to next provider or rule-based
        except Exception as e:
            err_str = str(e).lower()
            if "429" in str(e) or "quota" in err_str or "rate" in err_str or "resource_exhausted" in err_str:
                ...
                break  # don't try other providers, go straight to rule-based
    ...
    return jsonify({
        "response":      ai_result.get("response", f"Found {len(rows)} results."),
        "query_type":    ai_result.get("query_type", "bar"),
        "chart_type":    ai_result.get("chart_type", ai_result.get("query_type", "bar")),
        "chart_config":  ai_result.get("chart_config", {}),
        "title":         ai_result.get("title", prompt[:70]),
...
def _rule_based_query(prompt: str, time_filter: str = '1=1') -> dict:
    """
    NL → SQL rule-based engine.
    PRIMARY source: kpi_data + telecom_sites (where real uploaded data lives).
    FALLBACK: network_kpi_timeseries (if kpi_data empty).
    Detects which table has data and uses it.
    """
    import re
    p = prompt.lower()
    N = 10
    ...
    # Detect primary table — use kpi_data if it has rows, else network_kpi_timeseries
    try:
    ...
    if USE_KD:
        # kpi_data schema: site_id, kpi_name, value, date
        # Join with telecom_sites for zone/lat/lng
        GEO_JOIN = "LEFT JOIN telecom_sites ts ON LOWER(k.site_id) = LOWER(ts.site_id)"

        def _kd_site_query(kpi_names_and_aliases: list, order_col: str, order_dir: str = "DESC") -> str:
            """Build a pivot query from kpi_data for given KPI names.
            CRITICAL: always includes data_level='site' and kpi_name IN(...) for index usage."""
            case_parts = []
            kpi_names_for_in = []
            for kpi_name, alias in kpi_names_and_aliases:
                case_parts.append(
                    f"AVG(CASE WHEN k.kpi_name = '{kpi_name}' THEN k.value END) AS {alias}"
                )
        ...
        if 'rrc' in p or 'accessibility' in p or 'access' in p:
            sql = _kd_site_query([
                ...
                ("LTE Call Setup Success Rate", "lte_call_setup_sr"),
                ("DL PRB Utilization (1BH)", "dl_prb_util"),
            ], "lte_rrc_setup_sr", "ASC")
            return {"sql": sql, "query_type": "bar",
                    "title": f"RRC / Accessibility — Bottom {N} Sites",
                    "x_axis": "site_id", "y_axes": ["lte_rrc_setup_sr", "erab_setup_sr"],
                    "response": f"Showing {N} sites with lowest RRC Setup Success Rate."}
        ...
        if 'volte' in p:
            sql = _kd_site_query([
                ...
                ("VoLTE Traffic DL", "volte_dl"),
                ("VoLTE Traffic UL", "volte_ul"),
            ], "volte_traffic_erl", "DESC")
            return {"sql": sql, "query_type": "bar",
                    "title": f"Top {N} Sites — VoLTE Traffic",
                    "x_axis": "site_id", "y_axes": ["volte_traffic_erl"],
                    "response": f"Showing {N} sites by VoLTE Erlang traffic."}
        ...
        if 'handover' in p or ' ho ' in p or 'hsr' in p:
            sql = _kd_site_query([
                ...
                ("LTE RRC Setup Success Rate", "lte_rrc_setup_sr"),
                ("DL PRB Utilization (1BH)", "dl_prb_util"),
            ], "intra_freq_ho_sr", "ASC")
            return {"sql": sql, "query_type": "bar",
                    "title": f"Bottom {N} Sites — HO Success Rate",
                    "x_axis": "site_id", "y_axes": ["intra_freq_ho_sr"],
                    "response": f"Showing {N} sites with worst Handover Success Rate."}
        ...
        if 'drop' in p or 'cdr' in p:
            ...
            return {"sql": sql, "query_type": "mixed",
                    "title": f"Top {N} Call Drop Offenders",
                    "x_axis": "site_id", "y_axes": ["erab_drop_rate", "dl_prb_util"],
                    "response": f"Showing {N} sites with highest E-RAB call drop rate."}
        ...
        if 'prb' in p or 'congestion' in p or 'congested' in p or 'overload' in p:
            ...
            return {"sql": sql, "query_type": "mixed",
                    "title": f"Top {N} Most Congested Sites (PRB)",
                    "x_axis": "site_id", "y_axes": ["dl_prb_util", "ul_prb_util", "dl_cell_tput"],
                    "response": f"Showing top {N} sites by DL PRB Utilization."}
        ...
        if 'throughput' in p or 'tput' in p or 'speed' in p or 'mbps' in p:
            ...
            return {"sql": sql, "query_type": "mixed",
                    "title": f"{'Bottom' if order=='ASC' else 'Top'} {N} Sites — DL Throughput",
                    "x_axis": "site_id", "y_axes": ["dl_cell_tput", "ul_cell_tput"],
                    "response": f"{'Worst' if order=='ASC' else 'Best'} {N} sites by downlink throughput."}

        # ── Trend / time series ──────────────────────────────────────────────
        if 'trend' in p or 'over time' in p or 'history' in p or 'daily' in p:
            if 'rrc' in p:      kpi_name, alias = "LTE RRC Setup Success Rate", "rrc_sr"
            elif 'drop' in p:   kpi_name, alias = "E-RAB Call Drop Rate_1", "drop_rate"
            elif 'prb' in p:    kpi_name, alias = "DL PRB Utilization (1BH)", "dl_prb"
            elif 'tput' in p or 'throughput' in p: kpi_name, alias = "LTE DL - Cell Ave Throughput", "dl_tput"
            elif 'avail' in p:  kpi_name, alias = "Availability", "availability"
            else:               kpi_name, alias = "DL PRB Utilization (1BH)", "dl_prb"
            return {
                "sql": f"""
                    SELECT k.date::text AS date,
                           AVG(k.value) AS {alias},
                           MIN(k.value) AS min_val,
                           MAX(k.value) AS max_val
                    FROM kpi_data k
        ...
        if 'zone' in p or 'cluster' in p or 'cbd' in p or 'urban' in p or 'compar' in p:
            return {
                ...
                """,
                "query_type": "bar", "title": "Zone-wise KPI Comparison",
                "x_axis": "cluster", "y_axes": ["avg_prb", "avg_tput", "avg_drop"],
                "response": "Zone-level KPI comparison across the network."
            }
        ...
        if 'availab' in p or 'downtime' in p or 'uptime' in p:
            ...
            return {"sql": sql, "query_type": "bar",
                    "title": "Sites with Lowest Availability",
                    "x_axis": "site_id", "y_axes": ["availability"],
                    "response": "Sites with lowest network availability."}

        # ── Latency ──────────────────────────────────────────────────────────
        if 'latency' in p or 'delay' in p or 'ping' in p or 'लेटेंसी' in p:
            sql = _kd_site_query([
                ("Average Latency Downlink", "avg_latency"),
                ("LTE DL - Usr Ave Throughput", "dl_usr_tput"),
            ], "avg_latency", "DESC")
            return {"sql": sql, "query_type": "bar",
                    "title": f"Top {N} High Latency Sites",
                    "x_axis": "site_id", "y_axes": ["avg_latency"],
                    "response": f"Showing {N} sites with highest downlink latency."}

        # ── Data Volume ──────────────────────────────────────────────────────
        if 'volume' in p or 'data volume' in p or 'traffic' in p or 'डेटा' in p:
            sql = _kd_site_query([
                ("DL Data Total Volume", "dl_volume"),
                ("UL Data Total Volume", "ul_volume"),
                ("DL PRB Utilization (1BH)", "dl_prb_util"),
            ], "dl_volume", "DESC")
            return {"sql": sql, "query_type": "bar",
                    "title": f"Top {N} Sites by Data Volume",
                    "x_axis": "site_id", "y_axes": ["dl_volume", "ul_volume"],
                    "response": f"Showing {N} sites with highest DL data volume."}

        # ── CSSR / Call Setup ────────────────────────────────────────────────
        if 'cssr' in p or 'call setup' in p or 'setup success' in p:
            sql = _kd_site_query([
                ("LTE Call Setup Success Rate", "lte_cssr"),
                ("LTE E-RAB Setup Success Rate", "erab_setup_sr"),
                ("E-RAB Call Drop Rate_1", "erab_drop_rate"),
            ], "lte_cssr", "ASC")
            return {"sql": sql, "query_type": "bar",
                    "title": f"Bottom {N} Sites — Call Setup Success",
                    "x_axis": "site_id", "y_axes": ["lte_cssr", "erab_setup_sr"],
                    "response": f"Showing {N} sites with lowest Call Setup Success Rate."}

        # ── Users / Connected UE ─────────────────────────────────────────────
        if 'user' in p or 'ue' in p or 'connected' in p or 'यूजर' in p:
            sql = _kd_site_query([
                ("Ave RRC Connected Ue", "avg_rrc_ue"),
                ("Max RRC Connected Ue", "max_rrc_ue"),
                ("DL PRB Utilization (1BH)", "dl_prb_util"),
            ...
            return {"sql": sql, "query_type": "bar",
                    "title": f"Top {N} Sites by Connected Users",
                    "x_axis": "site_id", "y_axes": ["avg_rrc_ue", "max_rrc_ue"],
                    "response": f"Showing {N} sites with most connected users."}

        # ── Default: worst PRB sites ─────────────────────────────────────────
        sql = _kd_site_query([
            ("DL PRB Utilization (1BH)", "dl_prb_util"),
            ("LTE DL - Cell Ave Throughput", "dl_cell_tput"),
            ("E-RAB Call Drop Rate_1", "erab_drop_rate"),
        ...
        return {"sql": sql, "query_type": "mixed",
                "title": f"Top {N} Sites by PRB Utilization",
                "x_axis": "site_id", "y_axes": ["dl_prb_util", "dl_cell_tput", "erab_drop_rate"],
                "response": f"Showing top {N} sites ranked by PRB utilization."}
